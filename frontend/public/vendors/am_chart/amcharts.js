...
(function () {
  ...
  d.AxisBase = d.Class({
    ...
    checkPeriodChange: function (a, b, c, e, g) {
      ...
      return 'DD' == a && 'hh' != g && c - b < d.getPeriodDuration(a, e) - d.getPeriodDuration('hh', 1)
        ? !1
        : c != b
          ? !0
          : !1;
  ...
  d.ValueAxis = d.Class({
    ...
    draw: function () {
      ...
      if (isNaN(this.min) || isNaN(this.max) || !this.foundGraphs || Infinity == this.min || -Infinity == this.max)
        ...
        isNaN(w) || (x = w);
        isNaN(this.maxZoom) &&
          ((this.max = d.roundTo(this.max, this.maxDecCount)), (this.min = d.roundTo(this.min, this.maxDecCount)));
        h = {};
        h.precision = x;
    ...
    getDateMinMax: function () {
      this.minimumDate &&
        (this.minimumDate instanceof Date ||
          (this.minimumDate = d.getDate(this.minimumDate, this.chart.dataDateFormat, 'fff')),
        (this.min = this.minimumDate.getTime()));
      this.maximumDate &&
        (this.maximumDate instanceof Date ||
          (this.maximumDate = d.getDate(this.maximumDate, this.chart.dataDateFormat, 'fff')),
              ...
              'regular' == b &&
                (r &&
                  (isNaN(c[p])
                    ...
                    : (isNaN(h) ? (m.values.close = c[p]) : (m.values.close = h + c[p]),
                      (m.values.open = c[p]),
                      (c[p] = m.values.close))),
                'column' == a &&
                  (n.newStack && ((g[p] = NaN), (e[p] = NaN)),
      ...
      this.logarithmic &&
        (2 < Math.log(b) * Math.LOG10E - Math.log(a) * Math.LOG10E
          ...
          : ((a = Math.pow(10, Math.floor(Math.log(Math.abs(a)) * Math.LOG10E)) / 10),
            Math.pow(10, Math.floor(Math.log(Math.abs(this.min)) * Math.LOG10E)) / 10 < a &&
    ...
    coordinateToValue: function (a) {
      ...
      return !0 === this.logarithmic
        ? Math.pow(
            10,
            (d ? (!0 === e ? (b - a) / c : a / c) : !0 === e ? a / c : (b - a) / c) + Math.log(h) * Math.LOG10E,
          )
        : !0 === e
          ? d
            ? f - (a - b) / c
    ...
    handleSynchronization: function () {
      if (this.synchronizeWith) {
        ...
        e &&
          ...
          (this.step = a * e),
          (b = Math.abs(
            Math.log(Math.abs(Math.pow(10, Math.floor(Math.log(Math.abs(this.step)) * Math.LOG10E)))) * Math.LOG10E,
          )),
  ...
  d.RecItem = d.Class({
    construct: function (a, b, c, e, g, f, h, k, l, m, n, q) {
      ...
      P &&
        '' !== c &&
        ((ca =
          a.autoWrap && 0 === n ? d.wrappedText(z, c, w, t, r, ga, k, Math.abs(g), 0) : d.text(z, c, w, t, r, ga, k)),
        (ga = ca.getBBox()),
        (ha = ga.width),
        (Qa = ga.height));
      if ('H' == Pa) {
        ...
        'right' == Z
          ? ((G += ma + xa + ea),
            (v += na),
            J ? (f || (v -= r / 2 + 3), (G = G - (B + 4) - aa)) : ((G += B + 4 + u), (v -= 2), (G += aa)))
          : J
      ...
      for (L = P; L <= ia; L++) {
        ...
        if (m && void 0 == this.openField)
          for (
            var yb = L + 1;
            yb < e.length &&
            (!e[yb] || !(fb = e[L + 1].axes[f.id].graphs[b]) || !fb.values || ((Ta = fb.values.value), isNaN(Ta)));
            yb++
          );
        var S,
          R,
        ...
        if (Ia) {
          ...
          isNaN(ua) ||
            ((N = f.getCoordinate(ua, xb)), m && 'regular' != M && '100%' != M && ((Ta = ua), (ua = N = NaN)));
          m &&
          ...
          'start' == this.pointPosition &&
            ((ja -= Y / 2 + Math.floor((Xa * Y) / 2)), (wa = 0), (Ga = Math.floor(Y) + Math.floor(Xa * Y)));
          'end' == this.pointPosition &&
            ((ja += Y / 2 + Math.floor((Xa * Y) / 2)), (wa = Math.floor(Y) + Math.floor(Xa * Y)), (Ga = 0));
          if (Ob) {
                ...
                if (!((W < Qb && rb < Qb) || (W > Rb && rb > Rb))) {
                  ...
                  if (t) {
                    ...
                    isNaN(G.percentWidthValue) ||
                      ((K = (this.height * G.percentWidthValue) / 100), (R = ja - K / 2), (Ea = R + K / 2));
                    K = d.roundTo(K, 2);
                    ba = d.roundTo(ba, 2);
                    R < q &&
                      0 < K &&
                    ...
                    isNaN(G.percentWidthValue) ||
                      ((K = (this.width * G.percentWidthValue) / 100), (S = ja - K / 2), (ya = S + K / 2));
                    K = d.roundTo(K, 2);
                    ba = d.roundTo(ba, 2);
                    S < n + Z * H &&
                      0 < K &&
    ...
    drawLineGraph: function (a, b, c, e) {
      ...
      if (1 < a.length) {
        ...
        var z = g.negativeLineColor,
          ...
          y = g.baseCoord;
        0 !== g.negativeBase &&
          ((y = g.valueAxis.getCoordinate(g.negativeBase, f)), y > g.height && (y = g.height), 0 > y && (y = 0));
...
(function () {
  ...
  d.ChartCursor = d.Class({
    ...
    handleMouseDown: function () {
      ...
      if (this.mouseIsOver())
        if (
          ...
          !this.pan &&
            isNaN(this.mouse2X0) &&
            (isNaN(this.fx) || (this.mouseX0 = this.fx), isNaN(this.fy) || (this.mouseY0 = this.fy)),
    ...
    setIndex: function (a, b) {
      a != this.index &&
        ((this.index = a), b || this.isHidden || this.dispatchMovedEvent(this.mouseX, this.mouseY, 'changed'));
    },
    handleMouseOut: function () {
      this.enabled &&
        this.rolledOver &&
...
(function () {
  ...
  d.ChartScrollbar = d.Class({
      ...
      if (this.categoryAxis && 'ValueAxis' == this.categoryAxis.cname)
        ...
        if (k.parseDates && !k.equalSpacing) {
          if (
            ...
            0 < g &&
              b - k < g &&
              ((b = Math.round(k + (b - k) / 2)), (m = Math.round(g / 2)), (k = b - m), (b += m), (m = !0)),
            0 < e &&
              b - k > e &&
              ((b = Math.round(k + (b - k) / 2)), (m = Math.round(e / 2)), (k = b - m), (b += m), (m = !0)),
            b > a && (b = a),
            b - g < k && (k = b - g),
            k < h && (k = h),
            k + g > b && (b = k + g),
          ...
          if (g != this.start || this.end != b)
            k.startOnAxis &&
              (this.resizingRight && g == b && b++, this.resizingLeft && g == b && (0 < g ? g-- : (b = 1))),
              (this.start = g),
    ...
    zoomScrollbar: function (a) {
      if (
        (!(this.dragging || this.resizingLeft || this.resizingRight || this.animating) || a) &&
        this.dragger &&
        this.enabled
      ) {
        var b,
  ...
  d.TrendLine = d.Class({
    ...
    draw: function () {
      ...
      if (!(isNaN(e) || isNaN(g) || isNaN(f) || isNaN(f))) {
        ...
        q =
          (90 * Math.PI) / 180 -
          Math.asin(n / (((n * q) / Math.abs(n * q)) * Math.sqrt(Math.pow(n, 2) + Math.pow(q, 2))));
  ...
  d.AmLegend = d.Class({
    ...
    createEntry: function (a) {
      if (!1 !== a.visibleInLegend && !a.hideFromLegend) {
        ...
        c.accessible &&
          b.accessibleLabel &&
          ((l = d.massReplace(b.accessibleLabel, { '[[title]]': a.title })), c.makeAccessible(q, l));
        l = b.fontSize;
        m &&
