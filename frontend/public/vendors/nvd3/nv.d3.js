 ...
 */
  ...
  nv.models.distroPlot = function () {
    ...
    function calcBandwidth(x, type) {
      ...
      var n = x.length;
      return type === 'scott' ? Math.pow(1.059 * A * n, -0.2) : Math.pow(0.9 * A * n, -0.2);
    }
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        var areaEnter,
          distroplots = wrap.selectAll('.nv-distroplot-x-group').data(function (d) {
            return d;
          });
        ...
        obsWrap.attr('class', function (d) {
          return (
            ...
            (isOutlier(d) && plotType == 'box' ? 'nv-distroplot-outlier' : 'nv-distroplot-non-outlier')
          );
        });
        ...
        if (observationType !== false) {
          ...
          if (!showOnlyOutliers) {
            ...
            // show only outliers
            distroplots
              .selectAll('.nv-distroplot-outlier ' + (observationType == 'line' ? 'line' : 'circle'))
              .watchTransition(renderWatch, 'nv-distroplot: nv-distroplot-observation')
              .style('opacity', 1);
            distroplots
              .selectAll('.nv-distroplot-non-outlier ' + (observationType == 'line' ? 'line' : 'circle'))
              .watchTransition(renderWatch, 'nv-distroplot: nv-distroplot-observation')
              .style('opacity', 0);
          }
        }
        ...
        distroplots
          .selectAll('.nv-distroplot-observation' + (observationType == 'line' ? ' circle' : ' line'))
      ...
      selection.each(function (data) {
        ...
        if (align) {
          ...
          series.attr('transform', function (d, i) {
            return (
              'translate(' + xPositions[i % seriesPerRow] + ',' + (5 + Math.floor(i / seriesPerRow) * versPadding) + ')'
            );
          });
...
*/
  ...
  nv.models.heatMap = function () {
    ...
    function cellColor(d) {
      ...
      return (cellsAreNumeric() && !isNaN(colorVal)) || typeof colorVal !== 'undefined'
        ? colorScale(colorVal)
        : missingDataColor;
    }
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        if (showValues && !stacked)
          y.range(
            yRange || [y.domain()[0] < 0 ? valuePadding : 0, availableWidth - (y.domain()[1] > 0 ? valuePadding : 0)],
          );
    ...
    function chart(selection) {
      selection.each(function (data) {
        ...
        ticks
          ...
          .attr('class', function (d, i, j) {
            return (
              (getOpen(d, i) > getClose(d, i) ? 'nv-tick negative' : 'nv-tick positive') + ' nv-tick-' + j + '-' + i
            );
          })
          .attr('d', function (d, i) {
            return (
      ...
      selection.each(function (data) {
        ...
        function restoreBrush(visible) {
          filters.forEach(function (f) {
            ...
            if (f.hasOnlyNaN) {
              f.extent[1] =
                ((y[f.dimension].domain()[1] - brushDomain[0]) * (f.extent[1] - f.extent[0])) /
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        scatter.dispatch.on('elementMouseout.tooltip', function (evt) {
          tooltip.hidden(true);
          container
            .select('.nv-chart-' + scatter.id() + ' .nv-series-' + evt.seriesIndex + ' .nv-distx-' + evt.pointIndex)
            .attr('y1', 0);
          container
            .select('.nv-chart-' + scatter.id() + ' .nv-series-' + evt.seriesIndex + ' .nv-disty-' + evt.pointIndex)
            .attr('x2', distY.size());
        });
                ...
                .attr('transform', function () {
                  ...
                  if (e.depth === 0) return 'translate(' + (width / 2) * -1 + ',0)';
                    ...
                    if (rotation === 0) {
                      ...
                      return (
                        'rotate(' + centerAngle + ')translate(' + (y(e.y) + width + 5) + ',0)rotate(' + rotation + ')'
                      );
                    }
                  }
                });
